...
사람 검수: **{status["reviewed"]}/{status["total_errors"]}건**. 최소 30건 직접 검수 요구의 충족 여부: **{status["complete"]}**. 자동 제안은 실제 원인 또는 사람 검수로 간주하지 않는다. 본 보고서는 검수가 끝나기 전에는 최종 제출 완료본이 아니다.
...
"""
    ...
    write_json(
        root / "status.json",
        {
            ...
            "human_review": status,
            "submission_ready": status["complete"],
        },
    )
